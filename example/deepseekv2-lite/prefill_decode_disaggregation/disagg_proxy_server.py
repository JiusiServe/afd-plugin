...
run ``vllm bench serve``:
  ...
  4. decode the remaining tokens on the decoder (prompt + first token), streamed
     straight back as SSE (``vllm bench serve`` requires a streamed response).
...
async def completions(request: Request) -> StreamingResponse:
    ...
    if stream_options is not None:
        decode_req["stream_options"] = stream_options
    ...
    async def stream():
        yield f"data: {json.dumps(head_chunk, separators=(',', ':'))}\n\n".encode()
        async with decoder.stream("POST", "/v1/completions", json=decode_req) as resp:
            resp.raise_for_status()
