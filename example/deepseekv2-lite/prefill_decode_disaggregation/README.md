...
| 2   | Decode attention | `AFDAttentionWorker` · 18305 | PD `receiver` (KV owner) |
| 3   | Decode FFN       | `AFDFFNWorker` · 18305   | none (holds no KV cache)|

Client traffic goes to the **proxy on `18300`**, never to `18305` directly.
...
| Port  | Purpose                                  |
|-------|------------------------------------------|
| 18300 | proxy (client-facing endpoint)           |
| 18301 / 18302 | prefill producers (HTTP)         |
| 18305 | decode attention (HTTP)                  |
