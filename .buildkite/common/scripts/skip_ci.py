...
``resolve_ci_decision(changed_files, ...)`` buckets explicit file lists
(tests, dry-run).
...
class CiDecision:
    ...
    def skip_all_basis(cls, buckets: DiffBuckets, *, diff_range: str | None) -> bool:
        ...
        if not buckets.has_doc_changes and not buckets.has_skip_test_changes:
            return False
        # Skip-all needs a verifiable diff, unless docs alone already qualify.
        return diff_range is not None or buckets.has_doc_changes
...
def _is_skip_testcase_change(file_path: str, *, diff_range: str) -> bool:
    """Return True when a test file diff is skip-mark-only.

    Includes new-file cases with module-level pytest.mark.skip.
    """
    ...
    if match is not None and _PYTESTMARK_SKIP_RE.search(match.group(1)) is not None:
        _log(
            "skip-mark-only: new test file with module-level "
            f"pytest.mark.skip: {file_path}"
        )
        return True
    ...
    _log(
        "skip-mark-only: new test file without module-level "
        f"pytest.mark.skip: {file_path}"
    )
    return False
    ...
    if not buckets.has_l23_yaml_changes and not buckets.has_l45_yaml_changes:
        if buckets.has_skip_test_changes and diff_range is None:
            message = (
                ...
                "diff range is available to verify them "
                "(skip-all requires diff_range); run normal CI"
            )
        elif buckets.has_skip_test_changes:
            message = (
                "pytest skip-mark test changes did not qualify for "
                "skip-all and no ci yaml changed; run normal CI"
            )
        else:
            message = (
