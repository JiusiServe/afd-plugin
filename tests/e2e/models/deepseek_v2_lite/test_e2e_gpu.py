...
"""Opt-in GPU E2E tests for DeepSeekV2 AFD — 2A2F eager/graph matrix.

All tests use a 2A2F topology (2 attention + 2 FFN servers, needs >=4 GPUs) and
...
= 10 tests. Profiler is enabled purely through AFD_GPU_{ATTENTION,FFN}_PROFILER_*
env vars, which leak through runner.py's os.environ.copy() into the vllm worker
— no runner/source change required.

Skipped unless AFD_GPU_E2E_MODEL is set to a local DeepSeekV2-Lite model path.
"""
...
def _graph_capture_size() -> int:
    return int(os.environ.get("AFD_GPU_E2E_GRAPH_CAPTURE_SIZE", "8"))
...
def _run_e2e(
    *,
    ...
    model = _model_path()
    if len(gpus) < 4:
        pytest.skip(f"2A2F requires 4 GPUs; got {len(gpus)}")
    ...
    if graph:
        requests = capture_size * 2 if ubatch else capture_size
        command.extend(
            [
    ...
    if dbo:
        ...
        if ubatch:
            command.append("--expect-ffn-ubatch-cudagraph-replay")
...
def _enable_profiler(
    ...
    the vllm worker, where afd_plugin/compat/profiler.py reads them.
    """
...
def test_deepseek_v2_2a2f_eager():
    _run_e2e(
        gpus=_gpu_list(),
        api_port_base=int(os.environ.get("AFD_GPU_E2E_2A2F_EAGER_API_PORT", "18000")),
        afd_port=int(os.environ.get("AFD_GPU_E2E_2A2F_EAGER_AFD_PORT", "6239")),
    )
...
def test_deepseek_v2_2a2f_graph():
    _run_e2e(
        gpus=_gpu_list(),
        api_port_base=int(os.environ.get("AFD_GPU_E2E_2A2F_GRAPH_API_PORT", "18100")),
        afd_port=int(os.environ.get("AFD_GPU_E2E_2A2F_GRAPH_AFD_PORT", "6249")),
        graph=True,
    )
...
def test_deepseek_v2_2a2f_tp_eager():
    _run_e2e(
        gpus=_gpu_list(),
        api_port_base=int(
            os.environ.get("AFD_GPU_E2E_2A2F_TP_EAGER_API_PORT", "18200")
        ),
        afd_port=int(os.environ.get("AFD_GPU_E2E_2A2F_TP_EAGER_AFD_PORT", "6259")),
        tp=2,
    )
...
def test_deepseek_v2_2a2f_tp_graph():
    _run_e2e(
        gpus=_gpu_list(),
        api_port_base=int(
            os.environ.get("AFD_GPU_E2E_2A2F_TP_GRAPH_API_PORT", "18300")
        ),
        afd_port=int(os.environ.get("AFD_GPU_E2E_2A2F_TP_GRAPH_AFD_PORT", "6269")),
        graph=True,
        tp=2,
    )
...
def test_deepseek_v2_2a2f_dbo_eager():
    _run_e2e(
        gpus=_gpu_list(),
        api_port_base=int(
            os.environ.get("AFD_GPU_E2E_2A2F_DBO_EAGER_API_PORT", "18400")
        ),
        afd_port=int(os.environ.get("AFD_GPU_E2E_2A2F_DBO_EAGER_AFD_PORT", "6279")),
        dbo=True,
    )
...
def test_deepseek_v2_2a2f_dbo_graph():
    _run_e2e(
        gpus=_gpu_list(),
        api_port_base=int(
            os.environ.get("AFD_GPU_E2E_2A2F_DBO_GRAPH_API_PORT", "18500")
        ),
        afd_port=int(os.environ.get("AFD_GPU_E2E_2A2F_DBO_GRAPH_AFD_PORT", "6289")),
        graph=True,
        dbo=True,
    )
...
def test_deepseek_v2_2a2f_profile_eager(
    ...
    _run_e2e(
        gpus=_gpu_list(),
        api_port_base=int(
            os.environ.get("AFD_GPU_E2E_2A2F_PROFILE_EAGER_API_PORT", "18600")
        ),
        afd_port=int(os.environ.get("AFD_GPU_E2E_2A2F_PROFILE_EAGER_AFD_PORT", "6299")),
    )
...
def test_deepseek_v2_2a2f_profile_graph(
    ...
    _run_e2e(
        gpus=_gpu_list(),
        api_port_base=int(
            os.environ.get("AFD_GPU_E2E_2A2F_PROFILE_GRAPH_API_PORT", "18700")
        ),
        afd_port=int(os.environ.get("AFD_GPU_E2E_2A2F_PROFILE_GRAPH_AFD_PORT", "6309")),
        graph=True,
    )
    _assert_profiler_traces(attn_dir, ffn_dir)
...
def test_deepseek_v2_2a2f_tp_dbo_profile_eager(
    ...
    _run_e2e(
        gpus=_gpu_list(),
        api_port_base=int(
            os.environ.get("AFD_GPU_E2E_2A2F_TP_DBO_PROFILE_EAGER_API_PORT", "18800"),
        ),
        afd_port=int(
            os.environ.get("AFD_GPU_E2E_2A2F_TP_DBO_PROFILE_EAGER_AFD_PORT", "6319"),
        ),
        tp=2,
        dbo=True,
    )
...
def test_deepseek_v2_2a2f_tp_dbo_profile_graph(
    ...
    _run_e2e(
        gpus=_gpu_list(),
        api_port_base=int(
            os.environ.get("AFD_GPU_E2E_2A2F_TP_DBO_PROFILE_GRAPH_API_PORT", "18900"),
        ),
        afd_port=int(
            os.environ.get("AFD_GPU_E2E_2A2F_TP_DBO_PROFILE_GRAPH_AFD_PORT", "6329"),
        ),
        graph=True,
        tp=2,
        dbo=True,
