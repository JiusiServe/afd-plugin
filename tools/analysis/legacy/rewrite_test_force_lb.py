...
path = Path(
    "/mnt/d/cyj/afd/afd-plugin/tests/unit/compat/patches/"
...
class AscendFusedMoE:
    """Stand-in for vllm_ascend.ops.fused_moe.fused_moe.AscendFusedMoE."""
...
def build_fused_experts_input(*args: object, **kwargs: object) -> torch.Tensor:
    """Fake builder: returns the possibly swapped topk_ids for assertion."""
...
def force_lb_mod(monkeypatch: pytest.MonkeyPatch) -> types.ModuleType:
    ...
    module_name = "afd_plugin.compat.patches.force_load_balance"
