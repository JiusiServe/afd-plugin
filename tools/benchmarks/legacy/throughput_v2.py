#!/usr/bin/env python3
"""Build per-die throughput CSV/PNG files from AISBench result dirs."""
...
def main() -> None:
    ...
    file_name = sanitize_name(args.file_name)
    label = args.label.strip()
    label_name = sanitize_name(label)
    output_dir = Path(args.output_dir)
    csv_path = output_dir / f"throughput_{file_name}_{label_name}.csv"
    png_path = output_dir / f"throughput_{file_name}.png"

    rows = build_throughput_rows(records, die_count=args.die_count, label=label)
    output_dir.mkdir(parents=True, exist_ok=True)
    write_csv(csv_path, rows)
    plot_per_die_png(
        png_path,
        load_plot_series(output_dir, file_name),
        title=args.title,
    )

    total_tokens = sum(record.output_tokens for record in records)
    duration = rows[-1]["elapsed_s"] + 1 if rows else 0
    peak = max(row["tokens/s/die"] for row in rows) if rows else 0.0
    print(f"input: {result_file}")
    print(f"requests: {len(records)}")
    print(f"output_tokens: {total_tokens}")
    print(f"duration_s: {duration:.0f}")
    print(f"peak_tokens/s/die: {peak:.6f}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate throughput_<file-name>_<label>.csv and update "
            "throughput_<file-name>.png with all labels for that file name."
        )
    )
    parser.add_argument(
        "--file-name",
        required=True,
        help="Shared output name. Same file-name appends another line to the PNG.",
    )
    parser.add_argument(
        "--ais_bench_dir",
        "--ais-bench-dir",
        required=True,
        help="AISBench run directory name or path. Only this directory is parsed.",
    )
    parser.add_argument(
        "--label",
        required=True,
        help="Legend label for this run's line in the PNG.",
    )
    parser.add_argument(
        "--title",
        required=True,
        help="Title for throughput_<file-name>.png.",
    )
    parser.add_argument(
        "--input-root",
        action="append",
...
def first_float(flat: Mapping[str, Any], keys: Iterable[str]) -> float | None:
    for key in keys:
        parsed = parse_float(value_for_key(flat, key))
...
def write_csv(path: Path, rows: list[dict[str, Any]]) -> None:
    with path.open("w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(
            f,
            fieldnames=["label", "second", "elapsed_s", "tokens/s", "tokens/s/die"],
        )
        writer.writeheader()
        for row in rows:
            writer.writerow(
...
def load_plot_series(output_dir: Path, file_name: str) -> list[dict[str, Any]]:
    series: list[dict[str, Any]] = []
    for path in sorted(output_dir.glob(f"throughput_{file_name}_*.csv")):
        rows: list[dict[str, float]] = []
        label = label_from_csv_name(path, file_name)
        with path.open("r", encoding="utf-8-sig", newline="") as f:
            reader = csv.DictReader(f)
            for raw in reader:
                if raw.get("label"):
                    label = raw["label"]
                elapsed = parse_float(raw.get("elapsed_s"))
                per_die = parse_float(raw.get("tokens/s/die"))
                if elapsed is None or per_die is None:
                    continue
                rows.append({"elapsed_s": elapsed, "tokens/s/die": per_die})
        if rows:
            series.append({"label": label, "rows": rows, "mtime": path.stat().st_mtime})
    return sorted(series, key=lambda item: item["mtime"])


def label_from_csv_name(path: Path, file_name: str) -> str:
    prefix = f"throughput_{file_name}_"
    stem = path.stem
...
def plot_per_die_png(
    ...
    *,
    ...
    if not series:
        raise SystemExit("No CSV series found for plotting")

    plt.style.use("seaborn-v0_8-whitegrid" if "seaborn-v0_8-whitegrid" in plt.style.available else "ggplot")
    fig_width = max(11.0, min(18.0, 9.5 + 0.45 * len(series)))
    ...
    for idx, item in enumerate(series):
        ...
        y = [row["tokens/s/die"] for row in rows]
        ax.plot(
            x,
            y,
            label=item["label"],
    ...
    ax.set_title(title, fontsize=17, fontweight="semibold", pad=14)
    ax.set_xlabel("Elapsed time (s)", fontsize=12)
    ax.set_ylabel("Throughput per die (tokens/s/die)", fontsize=12)
