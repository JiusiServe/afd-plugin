...
Group files so tests can use ``follow_imports=skip`` without pulling that
setting into library code.
...
# Copied upstream patch bodies are large and intentionally keep upstream
# shapes; drop a prefix from this list when that tree is ready for mypy.
EXCLUDE = [
    r"afd_plugin/compat/patches/",
]
...
def group_files(changed_files: list[str]) -> dict[str, list[str]]:
    ...
    for changed_file in changed_files:
        ...
        for directory in separate_groups:
            ...
            if changed_file.startswith(("afd_plugin/", "tests/", "tools/")):
