...
- 当前四个 CAM 算子没有真实实现时，先使用打桩接口，接口形态以仓库根目录
  `cam_ops.py` 为准，用法参考 `test_cam_ops.py`。
...
- 第一版允许 `extra_config["use_stub_topk"] == true` 时在 Attention 侧生成 stub
  top-k，只用于打通 `AFDAsyncConnector` 和 CAM stub op 调用链；
- 如果关闭 `use_stub_topk` 且 Attention 侧拿不到真实 top-k 路由信息，必须
  fail fast。
...
```python
output = torch.ops.cam.cam_combine_recv(
...
`cam_combine_recv` 的输出 shape / dtype 应与 dispatch send 的 `hidden_states`
一致。
...
```python
dispatch_out = torch.ops.cam.cam_dispatch_recv(
    placeholder_tensor,
    comm_args,
    comm_id,
    max_seq_len,
...
当前 CAM 四算子没有真实实现时，第一版应提供或使用打桩接口：

- 参数签名必须与 `cam_ops.py` 一致；
- stub 只验证调用参数、shape、dtype 和返回结构；
- `cam_dispatch_send` / `cam_combine_send` 返回完成标记 tensor；
- `cam_dispatch_recv` 返回符合结构的占位 tensors；
- `cam_combine_recv` 返回与 Attention hidden states 同 shape / dtype 的 tensor；
- CPU-safe 单元测试使用 monkeypatch 或 `torch.library` fake op，不要求真实 NPU 通信；
- 真实 NPU smoke 测试再加载 `cam` / `cam_graph` / `torch_npu`。
...
第一版先使用 Attention 侧 stub top-k 打通接口：

```text
topk_ids:
  shape = [tokenNum, topKNum]
  dtype = int32
  值可以使用固定 0..topKNum-1、按 token round-robin，或其它确定性占位路由
...
- 验证 `AFDAsyncConnector` 的方法签名和 CAM 四算子参数顺序；
- 打通 Attention `dispatch_send -> combine_recv`；
- 打通 FFN `dispatch_recv -> combine_send`；
- 覆盖 connector-driven loop 不依赖 DP metadata 的控制流。

stub top-k 不用于验证模型输出正确性，也不应在真实 CAM 性能或精度测试中作为最终
方案。
...
如果 `use_stub_topk` 关闭且拿不到真实 top-k 信息，不能继续调用 CAM dispatch send，
应直接报错。
...
for layer in model.layers:
  ...
  # 第一版可以由 deterministic_stub_topk(...) 临时替代
...
- lazy import `torch_npu` / `cam` / `cam_graph`；
- 构造 `comm_args`；
- 保存 `comm_id`；
- 保存 topology 参数；
...
第一版没有真实 CAM 算子时，需要让单元测试和最小 runtime wiring 可以跑通。

推荐分两层：

```text
afd_plugin.compat.ascend.cam_ops
  ensure_cam_ops_available()
  register_stub_cam_ops_for_tests()
```

运行时：

- 如果 `cam` 包和 `torch.ops.cam.*` 真实存在，直接使用真实 op；
- 如果配置显式允许 stub，例如 `extra_config["use_stub_cam_ops"] == true`，注册
  stub；
...
- NPU opt-in 测试再验证真实 `torch_npu` / `cam` 环境。

stub 返回结构必须贴近真实接口，尤其是 `cam_dispatch_recv()` 的 7 元组，否则 FFN
runner 的 payload 逻辑无法提前覆盖。
...
```text
afdasyncconnector
  -> afd_plugin.connectors.npu.async_cam.AFDAsyncConnector
```
...
afdasyncconnector:
  ...
  - 第一版允许 use_stub_topk=true；关闭 stub 时必须有真实 top-k 来源
...
- 第一版在真实 top-k 前移之前，按 `extra_config["use_stub_topk"]` 生成
...
- 第一版允许 Attention 侧 stub top-k；
- 后续需要在 DeepSeek MoE 拆分点明确真实路由信息来源，并替换 stub。
...
Attention 侧拿到 `expertIds` / `expertScales`。第一版使用 deterministic stub top-k
只能打通接口和控制流，不能证明模型数值正确。
...
真实 CAM smoke 或精度测试应关闭 `use_stub_topk`，缺少真实 top-k 时 fail fast。
...
### Phase 1: contract / validation / CAM stub connector
...
- 使用 stub CAM op 跑通四个 connector 方法；
- 不接真实模型计算，只验证 payload 和调用顺序。

### Phase 2: Attention top-k 接入

- 先在 NPU Attention 模型 wrapper 中加入 deterministic stub top-k；
- stub top-k 只服务 `AFDAsyncConnector` 的 CAM stub op 和 connector-driven loop；
- `use_stub_topk=false` 时没有真实 top-k 必须 fail fast；
- 跑通 Attention `dispatch_send -> combine_recv` stub；
- 后续再把 stub 替换为 gate-on-attention 或模型层真实 top-k 捕获。
...
- 跑通 stub `dispatch_recv -> compute_ffn_output -> combine_send`。
...
- 第一版在 Attention 侧用 stub top-k 打通接口，后续明确真实 top-k 路由信息来源。
