...
In code, the control plane is a pluggable module rather than part of the connector interface. `P2pNcclAFDConnector` creates a `P2pNcclAFDControlPlain` instance — an implementation of the `AFDControlPlain` contract from `afd_plugin/connectors/base.py` — and exposes it as `connector.control_plane`. The Attention-side runner calls `control_plane.update_state_from_dp_metadata(...)` and `control_plane.send_dp_metadata_list(...)` before each step; the FFN worker loop blocks on `control_plane.recv_dp_metadata_list()`. The connector sets `ffn_step_trigger = Trigger.DP_METADATA`, meaning each FFN step is triggered by the arrival of a DP metadata payload (connectors without a DP metadata control plane leave `control_plane` as `None` and use `Trigger.CONNECTOR` to drive FFN steps from the connector itself).
