...
vLLM-Ascend package dependency in `pyproject.toml`. Before this document can be
normative, maintainers must record the source tag/commit used to refresh every
copied Ascend function, not only the container image that exercised it.
...
| [`config_validation.py`](../../../afd_plugin/compat/patches/config_validation.py): `EngineArgs.create_engine_config`, `VllmConfig.__post_init__` | For AFD-owned ubatching with a non-DeepEP backend, temporarily presents `deepep_low_latency` during upstream validation and restores the configured backend; non-AFD and non-ubatch paths delegate directly. | Imported by `register_afd`; same target/dev/unknown guard. Saves originals on upstream modules under AFD-specific attributes before installing wrappers. | [`test_config_validation.py`](../../../tests/unit/compat/patches/test_config_validation.py) covers AFD, non-AFD, repeated validation, and dev versions. | Remove when upstream validation can distinguish plugin-owned ubatching from native DeepEP ubatching. |
...
- the exact vLLM-Ascend source tag/commit is not recorded in package metadata;
