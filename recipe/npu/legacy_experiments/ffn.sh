...
# ---- NPU profiler (AFD_NPU_FFN_PROFILER_*) ----
...
export AFD_NPU_FFN_PROFILER_DIR="$SCRIPT_DIR/profile/ffn"   # 产物落这(绝对路径)
export AFD_NPU_FFN_PROFILER_SKIP_FIRST=50        # 跳过前 50 step(默认 1500);要稳态可调大
export AFD_NPU_FFN_PROFILER_ACTIVE=20            # 采集 20 step

nic_name="eth0"
local_ip="33.182.140.93"
...
VLLM_PID=$!
echo "$VLLM_PID" > ffn.pid
disown "$VLLM_PID"   # 脱离 shell,itask session 断了也活着;停的时候用 kill -TERM $(cat ffn.pid)
